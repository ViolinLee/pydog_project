...
class PyDog(object):
    ...
    def calibrate(self, load):
        ...
        def check_input():
            text = input("Press Enter to start the calibration, "
    ...
    def move_leg(self):
        ...
        for i, leg_id in enumerate(self.joints_ids):
            # self.servos.position(leg_id, degrees=servos_pos[i])  # When used another servo class for PCA9685
            self.servos.position(i, degrees=servos_pos[i])
    ...
    def process_demo_gait(self,  height, reverse, mode='trot', times=5, elapsed=10, duration=20):
        mode_gait_dict = {'trot': trot_gait, 'turn': turn_gait, 'gallup': gallup_gait, 'jump': jump_gait}
        try:
            print('In Gait Func')
            fr_xz_path, br_xz_path, bl_xz_path, fl_xz_path = mode_gait_dict[mode](height, reverse)
            cnt = 0
            print('In times While')
            while cnt < times:
